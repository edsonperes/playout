...
impl SystemStat {
    ...
    pub async fn stat(&self, config: &PlayoutConfig) -> Self {
        ...
        // Primeiro, tente encontrar o disco específico para o caminho de armazenamento
        for disk in &*disks {
            let mount_point = disk.mount_point().to_string_lossy();
            if mount_point.len() > 1 && config.channel.storage.starts_with(&*mount_point) {
        ...
        // Se não encontrou um disco específico, use o disco raiz
        if !found_disk {
            for disk in &*disks {
                let mount_point = disk.mount_point().to_string_lossy();
                if mount_point == "/" {
