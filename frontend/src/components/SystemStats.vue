<template>
    <div class="grid grid-cols-1 xs:grid-cols-2 border-4 rounded-md border-primary text-left shadow-sm min-w-[320px] md:min-w-[728px] max-w-[960px]">
        ...
        <div v-if="sysStat.storage?.total > 0" class="p-4 border border-primary">
            <div class="text-xl">{{ t('system.storage') }}</div>
            <div class="grid grid-cols-2 gap-2">
                <div>
                    <strong>{{ t('system.total') }}:</strong> {{ fileSize(sysStat.storage?.total) }}
                </div>
                <div>
                    <strong>{{ t('system.free') }}:</strong> {{ fileSize(sysStat.storage?.used) }}
                </div>
            </div>
        </div>
        <div v-else class="p-4 border border-primary">
            <div class="text-xl">{{ t('system.storage') }}</div>
            <div class="text-sm">{{ t('media.notExists') }}</div>
        </div>
        <div class="p-4 border border-primary">
            <div class="text-xl">
        ...
        <div v-if="sysStat.storage?.path" class="p-4 border border-primary">
            <div class="text-xl">{{ t('system.device') }}</div>
            <div v-if="sysStat.storage" class="grid grid-cols-1 gap-2">
                <div>
                    {{ sysStat.storage?.path }}
                </div>
            </div>
        </div>
        <div v-else class="p-4 border border-primary" />
