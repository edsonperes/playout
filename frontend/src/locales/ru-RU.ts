export default {
    ...
    config: {
        ...
        help: 'Help',
        generalHelp: 'Sometimes it can happen that a file is corrupt but still playable. This can produce a streaming error for all following files. The only solution in this case is to stop ecast and start it again.',
        stopThreshold: 'The threshold stops ecast if it is asynchronous in time above this value. A number below 3 can cause unexpected errors.',
        ...
        In production, don't serve HLS playlists with ecast; use Nginx or another web server!`,
        outputParam: 'HLS segment and playlist paths are relative.',
        restartTile: 'Перезапуск Playout',
        restartText: 'Перезапустить ecast для применения изменений?',
        ...
        sharedStorage: 'ecast работает внутри контейнера, используйте один и тот же корень хранилища для всех каналов!',
        timezone: 'Timezone',
    },
    user: {
